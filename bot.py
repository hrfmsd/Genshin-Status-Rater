...
bot = commands.AutoShardedBot(
    ...
    shard_count=SHARDS,
    max_messages=None,
    activity=discord.Game(name='-debug'),
    help_command=None
)
...
async def rate(ctx):
    ...
    for i in range(RETRIES + 1):
        try:
            calls += 1
            # OCR解析
            success, text = await rs.ocr(url, i + 1, lang)
    ...
    # 解析不明エラー
    if not results or not ideal_results:
        await send(ctx, msg=lang.err_unknown)
        return
    ...
    msg_cr = f'{results[2][1]:.1%}%'
    msg_ideal_cr = f'{ideal_results[1]:.1%}'
    msg_cd = f'{results[1][1]:.1%}%'
    msg_ideal_cd = f'{ideal_results[2]:.1%}'
    embed.add_field(name=f'**{lang.score}：{score}{lang.score_suffix}**',
                    value=f'> 理想値とのダメージ差：{ideal_results[3]:.2%}\n\n', inline=False)
    embed.add_field(name=f'**{lang.atk}**', value=f'> 現在値：{msg_atk}\n> 理想値：{mgs_ideal_atk}\n')
    embed.add_field(name=f'**{lang.cr}**', value=f'> 現在値：{msg_cr}\n> 理想値：{msg_ideal_cr}\n')
    embed.add_field(name=f'**{lang.cd}**', value=f'> 現在値：{msg_cd}\n> 理想値：{msg_ideal_cd}\n')
